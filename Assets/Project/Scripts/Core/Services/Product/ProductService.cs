...
{
    /// <summary>
    /// Shared product lifecycle service for both Software and Hardware families.
    /// Implements IProductService for all family-parameterized lifecycle operations.
    /// Software-specific metrics methods are also exposed on this class.
    /// Hardware-specific metrics live on HardwareMetricsService.
    ...
    public sealed class ProductService : IProductService
    {
        ...
        public ProductProfile CreateProductProfile(CreateProductRequest request, GameDateTime currentDate)
        {
            var profile = new ProductProfile
            {
                ...
                // Products require post-launch support resourcing for MVP.
                RequiresSupport                        = true
            };

            DebugLogger.Log(DebugCategory.Simulation,
                $"[ProductService] Product profile created. Id: {profile.Id}, Name: {profile.Name}, "
                + $"Category: {profile.Category}, Family: {profile.Family}");
            ...
            if (family == ProductFamily.Hardware)
            {
                switch (phase)
                {
                    case ProductStatus.InConcept:
                        baseHours = tuning.HardwareConceptPhaseBaseHours;
                        break;
                    case ProductStatus.InPrototype:
                        baseHours = tuning.HardwarePrototypePhaseBaseHours;
                        break;
                    case ProductStatus.InManufacturingPrep:
                        baseHours = tuning.HardwareManufacturingPrepPhaseBaseHours;
                        break;
                    default:
                        // No work required for non-work phases.
                        return 0;
                }
            }
            else
        ...
        public ProductStatus? GetNextPhase(ProductStatus currentPhase, ProductFamily family)
        {
            if (family == ProductFamily.Hardware)
            {
                // Hardware lifecycle: InConcept → InPrototype → InManufacturingPrep → ReadyForLaunch (gate, no auto-transition after).
                switch (currentPhase)
                {
                    case ProductStatus.InConcept:
                        return ProductStatus.InPrototype;
                    case ProductStatus.InPrototype:
                        return ProductStatus.InManufacturingPrep;
                    case ProductStatus.InManufacturingPrep:
        ...
        public AssignmentType GetAssignmentTypeForPhase(ProductStatus phase, ProductFamily family)
        {
            if (family == ProductFamily.Hardware)
            {
                switch (phase)
                {
                    ...
                    case ProductStatus.InPrototype:
                        return AssignmentType.HardwarePrototyping;
                    case ProductStatus.InManufacturingPrep:
                        return AssignmentType.ManufacturingSetup;
        ...
        public bool IsWorkPhase(ProductStatus status)
        {
            return status == ProductStatus.InConcept
                ...
                || status == ProductStatus.InPrototype
                || status == ProductStatus.InManufacturingPrep;
        }
        ...
        public ProductFamily DeriveFamily(ProductCategory category)
        {
            switch (category)
            {
                case ProductCategory.HardwarePlatform:
                case ProductCategory.Peripheral:
                case ProductCategory.LaptopDesktopDevice:
                case ProductCategory.ServerDevice:
                    return ProductFamily.Hardware;
                default:
            ...
            Random random)
        {
            ...
            DebugLogger.Log(DebugCategory.Simulation,
                $"[ProductService] Daily software metrics updated. ProductId: {productState.ProductId}, "
